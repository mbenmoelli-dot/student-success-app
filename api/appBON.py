...
app = FastAPI(title="Student Success API", version="3.3.0")
...
def select_scenario(payload: dict) -> str:
    has_g1 = payload.get("G1") is not None and payload.get("G1", 0) > 0
    has_g2 = payload.get("G2") is not None and payload.get("G2", 0) > 0
