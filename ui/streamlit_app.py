...
st.sidebar.markdown("""
- **S2** : G1 + G2 disponibles (Fiabilité : 92%)
...
# Notes - Section séparée avec checkboxes
st.markdown("---")
st.markdown("## 📊 Notes (optionnelles)")
st.markdown("*Cochez si la note est disponible, puis saisissez la valeur (même 0/20 est une note valide).*")

col_g1, col_g2, col_info = st.columns(3)

with col_g1:
    g1_disponible = st.checkbox("G1 disponible ?", value=False)
    if g1_disponible:
        G1 = st.number_input("G1 - Moyenne Trimestre 1", 0, 20, 10, help="Note sur 20")
    else:
        G1 = None
    
with col_g2:
    g2_disponible = st.checkbox("G2 disponible ?", value=False)
    if g2_disponible:
        G2 = st.number_input("G2 - Moyenne Trimestre 2", 0, 20, 10, help="Note sur 20")
    else:
        G2 = None

# Vérification de cohérence
erreur_coherence = False
if g2_disponible and not g1_disponible:
    erreur_coherence = True

with col_info:
    st.markdown("### 🎯 Scénario prévu")
    if erreur_coherence:
        st.error("❌ G1 doit être saisi avant G2")
    elif g1_disponible and g2_disponible:
...
if st.button("🔮 Prédire la réussite", type="primary", use_container_width=True):
    ...
    # Vérifier la cohérence G1/G2
    if erreur_coherence:
        st.error("❌ Impossible de prédire : vous devez saisir G1 avant G2")
    else:
        ...
        # Ajouter G1/G2 seulement si disponibles (même si = 0)
        if g1_disponible:
            payload["G1"] = G1
        if g2_disponible:
            payload["G2"] = G2
